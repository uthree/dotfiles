...
## Rustについて
Rustは標準で`cargo`がパッケージマネージャやフォーマッターを提供しているので、それを使用してください。
作業が完了した段階で、`cargo clippy`を使用してコードの可読性を高めてください。

## ファイルの操作について
一時ファイルを作って検証をする際はカレントディレクトリに作ってください。`/tmp/`などのルート直下に作るとシステムで必要なファイルと干渉する可能性があります。
